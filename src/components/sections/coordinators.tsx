...
import { DEVELOPING_TEAM } from "@/data/developingTeam";
...
export function Coordinators() {
  ...
  /*
   * Don't hide the section when the API is temporarily loading.
   * If there are no coordinators and no data yet, simply render nothing.
   */
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3 sm:gap-3 md:grid-cols-4 md:gap-4 lg:gap-6">
        {DEVELOPING_TEAM.map((member) => (
          <article
            key={member.id}
            className="group relative overflow-hidden rounded-lg glass glass-hover"
          >
            {/* Team photo */}
